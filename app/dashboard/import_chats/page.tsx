...
export default function ImportPeoplePage() {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            const backendUrl = getBackendUrl();
            const response = await fetch(`${backendUrl}/api/linked-in-chats/import`, {
                method: 'POST',
                body: formData,
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            if (!response.ok) {
                ...
                const errorMessage = responseBody.message || 'Failed to upload LinkedIn chats file. Please try again.';
            ...
            if (result.success === true) {
                setSuccessMessage(result.message || 'Chats imported successfully!');
                setFile(null); // Clear the file
                setSelectedProfile(''); // Clear the selected profile
                setError(null); // Clear any existing error
            } else {
                // Handle errors in the response
                console.log('Error: ', result.message || 'An error occurred while importing the chats.');
                setError(result.message || 'An error occurred while importing the chats.');
        ...
        <div className="min-h-screen bg-gray-100">
            ...
            <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                <div className="px-4 py-6 sm:px-0">
                    <h1 className="text-4xl font-bold mb-8 text-[#cc4c5c]">Import LinkedIn Chats</h1>
